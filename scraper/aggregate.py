...
# NOTE: stats.json and sources.json are owned by scripts/build_registry.py
# (the real 630-source free-API registry). The aggregator only refreshes live
# market data and stamps each file with cached_at + ttl for freshness display.
ticker['_meta'] = {'cached_at': now, 'ttl_seconds': TTL}
market['_meta'] = {'cached_at': now, 'ttl_seconds': TTL}

outputs = [
    ('live/ticker.json', ticker),
    ('live/market.json', market),
    ('deal/intelligence.json', {
        '_meta': {'cached_at': now, 'ttl_seconds': TTL},
        **{k: results.get(k, {}) for k in ['fema_disasters', 'fed_register', 'weather_alerts', 'fdic_failures', 'earthquakes', 'bls_cpi']},
    }),
]
manifest = {'generated': now, 'ttl_seconds': TTL, 'files': []}
for path, data in outputs:
    with open(os.path.join(OUT, path), 'w') as f:
        json.dump(data, f, indent=2, default=str)
    manifest['files'].append({'file': path, 'cached_at': now, 'fresh': True})
    print(f"  Wrote {path}")

with open(os.path.join(OUT, 'live', 'manifest.json'), 'w') as f:
    json.dump(manifest, f, indent=2)
print("  Wrote live/manifest.json")
print(f"\nDone! {results['_meta']['source_count']} live sources fetched at {now}.")
